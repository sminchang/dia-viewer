...
| xenia-ai/tag-platform 실측 (20~21노드/35~37엣지) | 12~16 | 이 규모의 정상 범위 |
...
**Cleanup 패스(OFF 뷰 한정)**: 슬롯 탐색은 중심 간 직선 프록시로 교차를 세는데, 실제 직각 라우터가 똑같이 깔끔하게 그리는 배치를 과대평가해 노드를 엉뚱하게 보낸다(sink가 호출자 옆에 둘 수 있는데 경계로 밀림). 최적해를 고른 뒤 **실제 라우터로** 정렬 이동(이웃과 같은 행/열의 빈 칸)을 재평가해 교차 안 늘고 길이 줄면 수락 — 수동 드래그 자동화. 라우트 예산 28회로 제한(레이아웃 ~2초). ON(주석) 뷰는 라벨 배치 우선이라 제외. 효과: xenia-ai/tag-platform OFF 교차 11/13 → **8/8**.
